    ...
    <div ref={ref} className="relative w-full max-w-md">
      <div className="relative">
        ...
        <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)}
          ...
          className="w-full h-9 pl-9 pr-3 text-sm bg-transparent border border-border rounded-full text-fg placeholder:text-fg-muted outline-none focus:border-border-focus transition-colors" />
      </div>
      ...
      {open && results.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-bg border border-border rounded-2xl shadow-2xl max-h-[320px] overflow-y-auto z-50">
          {results.slice(0, 20).map((r, i) => (
            <button key={i} onClick={() => { onNavigate(r.bookNumber, r.chapter); setOpen(false); setQuery(''); }}
